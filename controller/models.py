...
class RewardModel(nn.Module):
    ...
    def __init__(
        ...
        hidden_dim = hidden_dim if hidden_dim is not None else 2*(state_dim + observation_dim)

        self.mlp_layers = nn.Sequential(
            nn.Linear(state_dim + observation_dim, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, hidden_dim),
            nn.ReLU(),
